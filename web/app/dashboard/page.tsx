...
const statusColors: Record<string, string> = {
  ...
  active:   "bg-[#4ECDC4]/20 text-[#4ECDC4]",
  ...
  return (
    <div className="min-h-screen bg-[#0A0E1A]">
      ...
      <nav className="border-b border-white/10 px-8 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <Link
            ...
            className="bg-[#F5A623] text-[#0A0E1A] font-display font-700 text-sm px-4 py-2 rounded-lg hover:bg-[#F5A623]/90 transition-colors"
          >
            + New Assessment
          </Link>
          <button
      ...
      <main className="max-w-6xl mx-auto px-8 py-10">
        {/* Header */}
        <div className="mb-10">
          <h1 className="font-display text-3xl font-800 text-white mb-1">Assessments</h1>
          <p className="text-white/50 text-sm">Manage your hiring assessments and view results</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-10">
          {[
            ...
            <div key={stat.label} className="bg-white/5 border border-white/10 rounded-xl p-5">
              <p className="font-display text-3xl font-800 text-white mb-1">{stat.value}</p>
        ...
        {loading ? (
          ...
          <div className="border border-white/10 border-dashed rounded-2xl py-24 text-center">
            <p className="text-white/30 font-display text-lg mb-4">No assessments yet</p>
            <Link
              href="/assessments/new"
              className="bg-[#F5A623] text-[#0A0E1A] font-display font-700 text-sm px-6 py-3 rounded-lg hover:bg-[#F5A623]/90 transition-colors"
            >
              ...
              <thead>
                <tr className="border-b border-white/10">
                  {["Title", "Tests", "Candidates", "Status", ""].map((h) => (
                    <th key={h} className="text-left text-white/40 text-xs font-600 uppercase tracking-wider px-5 py-3.5">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {assessments.map((a) => (
                  <tr key={a.id} className="border-b border-white/5 hover:bg-white/3 transition-colors">
                    <td className="px-5 py-4">
                      <p className="text-white font-600 text-sm">{a.title}</p>
                      <p className="text-white/40 text-xs mt-0.5 font-mono">{a.id.slice(0, 8)}</p>
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {a.tests.length > 3 && (
                          <span className="text-white/30 text-xs">+{a.tests.length - 3}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <span className="font-mono text-white/70 text-sm">{a.candidate_count}</span>
                    </td>
                    <td className="px-5 py-4">
                      ...
                      <div className="flex items-center gap-3 justify-end">
                        {a.status === "draft" && (
                          <button
                            ...
                            className="text-[#4ECDC4] text-xs hover:underline"
                          >
                            Activate
                          </button>
                        )}
