...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-brand-surface flex">
      {/* Left panel */}
      <div className="hidden lg:flex flex-col justify-between w-[480px] p-12 bg-brand-dark">
        <div>
        ...
        <div>
          <blockquote className="text-white/80 text-lg font-display leading-relaxed mb-6">
            &ldquo;We screened 800 applicants in 4 days. TalentCheck gave us a ranked list and we hired the top 12 within a week.&rdquo;
          </blockquote>
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-brand-amber/20 flex items-center justify-center">
              <span className="text-brand-amber font-display font-700 text-sm">BK</span>
            </div>
            <div>
        ...
        <div className="flex gap-6">
          {[
            ...
            <div key={stat.label}>
              <p className="font-display text-2xl font-800 text-brand-amber">{stat.value}</p>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          <div className="mb-10 lg:hidden">
            <span className="font-display text-xl font-800 text-brand-dark tracking-tight">
              Talent<span className="text-brand-amber">Check</span>
            </span>
          </div>

          <h1 className="font-display text-3xl font-800 text-brand-dark mb-2">Welcome back</h1>
          <p className="text-brand-muted text-sm mb-10">
            Don&apos;t have an account?{" "}
            <Link href="/register" className="text-brand-amber hover:underline font-600">
              Create one
            </Link>
          </p>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-brand-amber text-white font-display font-700 py-3.5 rounded-lg hover:bg-brand-amber/90 transition-colors disabled:opacity-60 mt-2 shadow-card"
            >
